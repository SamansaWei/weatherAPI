/*
//點選縣市會變紅變粗
var countiesEles = document.querySelectorAll('.counties')

countiesEles.forEach(function (redEle,index) {
	redEle.addEventListener('click',function () {
		var activeTagEle = document.querySelector('.counties.counties-red');
		activeTagEle.classList.remove('counties-red');
		redEle.classList.add('counties-red');
	});
});

// 點選縣市區域點變紅並閃爍
var northlabEles = document.querySelectorAll('.northlab-run')
var centrallabEles = document.querySelectorAll('.centrallab-run')
var southlabEles = document.querySelectorAll('.southlab-run')
var eastlabEles = document.querySelectorAll('.eastlab-run')
var outerlabEles = document.querySelectorAll('.outerlab-run')

var poinorthEle = document.querySelector('.point.north')
var poicentralEle = document.querySelector('.point.central')
var poisouthEle = document.querySelector('.point.south')
var poieastEle = document.querySelector('.point.east')
var poiouterEle = document.querySelector('.point.outer')

northlabEles.forEach(function (runEle,index) {
	runEle.addEventListener('click',function(){
		var pointTagEle = document.querySelector('.point.point-run');
		pointTagEle.classList.remove('point-run')
		poinorthEle.classList.add('point-run')
	});
});

centrallabEles.forEach(function (runEle,index) {
	runEle.addEventListener('click',function(){
		var pointTagEle = document.querySelector('.point.point-run');
		pointTagEle.classList.remove('point-run')
		poicentralEle.classList.add('point-run')
	});
});

southlabEles.forEach(function (runEle,index) {
	runEle.addEventListener('click',function(){
		var pointTagEle = document.querySelector('.point.point-run');
		pointTagEle.classList.remove('point-run')
		poisouthEle.classList.add('point-run')
	});
});

eastlabEles.forEach(function (runEle,index) {
	runEle.addEventListener('click',function(){
		var pointTagEle = document.querySelector('.point.point-run');
		pointTagEle.classList.remove('point-run')
		poieastEle.classList.add('point-run')
	});
});

outerlabEles.forEach(function (runEle,index) {
	runEle.addEventListener('click',function(){
		var pointTagEle = document.querySelector('.point.point-run');
		pointTagEle.classList.remove('point-run')
		poiouterEle.classList.add('point-run')
	});
});
...
// 代入天氣資料
function downloadWeather(locationNam) {
	const conEle = document.querySelector('.details');
	let cardss = '';
	let url = 'https://opendata.cwb.gov.tw/api/v1/rest/datastore/F-C0032-001?Authorization=CWB-388946E9-C3E6-481B-A566-33036E545A11';
	var testNum = 0; // 0~21
	...
	fetch(url)
		...
		.then(function (data) {
			var locationMetData = data.records.location;

			// 找縣市名稱
			for (let i = 0; i < locationMetData.length; i++) {
				if (locationMetData[i].locationName === locationNam) {
					testNum = i;
					break;
				}
			}

			var countiesNameData = locationMetData[testNum].locationName;
			var tempData = locationMetData[testNum].weatherElement[2].time[0].parameter.parameterName;
			var feelData = locationMetData[testNum].weatherElement[3].time[0].parameter.parameterName;
			var skyData = locationMetData[testNum].weatherElement[0].time[0].parameter.parameterName;
			var rainWordData = locationMetData[testNum].weatherElement[1].time[0].parameter.parameterName;

			// 依條件改變氣候動畫
			if ( skyData.indexOf('雪') !== -1) {
				changeWeather(weather[0]);
			   } else if ( skyData.indexOf('雨') !== -1 || skyData.indexOf('陰') !== -1) {
				rainWordData < 50 ? changeWeather(weather[1]) : changeWeather(weather[2]);
			   } else if ( skyData.indexOf('雷') !== -1){
				changeWeather(weather[3]);
			   } else if ( skyData.indexOf('晴') !== -1){
			...
			cardss += `
			<div class="details">
				<div class="counties-name">${countiesNameData}</div>
				<div class="temp">${tempData}<span>c</span>
				</div>
				<div class="feel">${feelData}</div>
				<div class="right">
					<div id="sky">${skyData}</div>
					<div id="rain-word">降雨 <span>${rainWordData}%</span></div>
				</div>
				<div id="summary"></div>
			</div>`;

			conEle.innerHTML = cardss;
		})
}
...
countiesEles.forEach(function (redEle) {
	redEle.addEventListener('click', function () {
		downloadWeather(redEle.textContent);
		document.querySelector('.counties.counties-red').classList.remove('counties-red');
		redEle.classList.add('counties-red');
		document.querySelector('.point.point-run').classList.remove('point-run');
		//console.log(redEle.textContent);
		switch (redEle.classList[1]) {
			case 'northlab-run':
				document.querySelector('.point.north').classList.add('point-run');
				break;
			case 'centrallab-run':
				document.querySelector('.point.central').classList.add('point-run');
				break;
			case 'southlab-run':
				document.querySelector('.point.south').classList.add('point-run');
				break;
			case 'eastlab-run':
				document.querySelector('.point.east').classList.add('point-run');
				break;
			case 'outerlab-run':
				document.querySelector('.point.outer').classList.add('point-run');
				break;
			default: break;
		}
	});
